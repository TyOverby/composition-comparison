...
type alias Model = { howMany: Counter.Model, others: Dict Int Counter.Model }

init : Model
init = { howMany = Counter.init, others = Dict.empty }
...
updateSubcomponent msg maybeModel =
  case maybeModel of 
    Nothing -> Just (Counter.update msg 0)
    Just model_for_other  -> Just (Counter.update msg model_for_other)
...
update appMsg model =
  case appMsg of
    ...
    ForKey { msg, which } -> 
      let 
          others = Dict.update which (updateSubcomponent msg) model.others 
      in
      { model | others = others }
  ...
  let 
      a = 
        case Dict.get key models  of 
          Just model -> Counter.view model 
          Nothing -> Counter.view Counter.init
  in 
  Html.map (mapKey key) a
